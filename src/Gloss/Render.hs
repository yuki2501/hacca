...
renderArray cellColor cellSize field = let field4Render = activeCellIndex field
  ...
  where
    ...
    cellTranslate (i,j) = translate (fromIntegral ((j*cellSize)) - fromIntegral(((\(Rp.Z Rp.:._ Rp.:.j)-> j)(Rp.extent field))`div`2*cellSize))(fromIntegral((((\(Rp.Z Rp.:.i Rp.:._) -> i)(Rp.extent field))`div`2*cellSize)) + fromIntegral(-(i*cellSize)))
